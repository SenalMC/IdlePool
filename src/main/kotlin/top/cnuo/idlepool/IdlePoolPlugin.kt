...
class IdlePoolPlugin : JavaPlugin() {
    ...
    override fun onEnable() {
        saveDefaultConfig(); listOf("pools.yml","rewards.yml","message.yml","en.yml").forEach(::saveBundledFile)
        ...
        pools = PoolRepository(this); rewardPlans = RewardPlanRepository(this); snapshots = SnapshotRepository(this); reloadIdlePool()
        ...
        val rewards = RewardService(this, rewardPlans, store, economy); sessions = SessionManager(this, store, rewards, visuals, inbox)
        ...
        listOf(poolGui, inbox, adminGui, rewardAdminGui, PoolPresenceListener(this, pools, poolGui, sessions, store)).forEach { server.pluginManager.registerEvents(it, this) }
        val command = AfkPoolCommand(this, pools, poolGui, sessions, adminGui, rewardAdminGui, inbox, DoctorService(pools, rewardPlans, visuals, economy), PluginInfoService(this, visuals, updater))
        ...
        if (server.pluginManager.isPluginEnabled("PlaceholderAPI")) {
            runCatching { IdlePoolPlaceholderExpansion(this, sessions, store).register() }
    ...
    override fun onDisable() {
        if (::sessions.isInitialized) sessions.close(); if (::adminGui.isInitialized) adminGui.close(); if (::rewardAdminGui.isInitialized) rewardAdminGui.close(); if (::store.isInitialized) store.close()
    }

    fun reloadIdlePool() {
        reloadConfig(); Messages.load(this); val retention = DurationParser.parse(config.getString("progress.default-retention", "7d"))
        if (::pools.isInitialized) pools.reload(retention); if (::rewardPlans.isInitialized) rewardPlans.reload(); if (::snapshots.isInitialized) snapshots.reload()
    }
