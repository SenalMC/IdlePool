...
- 新增今日、本周、本月、总计统计与排行榜 GUI。
- 新增独立概率、权重单抽、权重多抽及周期保底。
- 新增限时倍率活动，可独立调整进度、物品与货币倍率。
- 新增异常领取复核中心、管理员审计和奖励流水 CSV 导出。
- 新增玩家奖励历史、管理员流水筛选和更多 PlaceholderAPI 变量。
- 新增可选 BossBar 状态显示。
- 新增 Bukkit Services API 与开始、结束、周期、里程碑、发奖和领取事件。
- 数据库与 YAML schema 升级至 3，并保留自动迁移和缺失语言键回退。
- 新增 GitBook 兼容完整 Wiki。
