...
public final class UpdateChecker {
    ...
    public void checkAsync() {
        if (!plugin.getConfig().getBoolean("update-check.enabled", true)) {
            result.set(new Result(State.DISABLED, "", Messages.raw("update.disabled")));
            return;
        }
        ...
        result.set(new Result(State.CHECKING, "", Messages.raw("update.checking")));
        ...
        try {
            ...
            fail(Messages.raw("update.invalid-url"));
            return;
        }
        ...
        client.sendAsync(request, HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8))
                ...
                .exceptionally(failure -> {
                    Throwable cause = failure instanceof CompletionException && failure.getCause() != null
                            ? failure.getCause()
                            : failure;
                    fail(Messages.raw("update.request-failed", java.util.Map.of("error", readableError(cause))));
                    return null;
                });
    }
    ...
    private void acceptResponse(HttpResponse<String> response) {
        if (response.statusCode() != 200) {
            fail(Messages.raw("update.http-error", java.util.Map.of("status", Integer.toString(response.statusCode()))));
            return;
        }
        ...
        if (remoteVersion.length() > 64 || !remoteVersion.matches("v?[0-9]+(?:\\.[0-9]+)*(?:-[0-9A-Za-z.-]+)?")) {
            fail(Messages.raw("update.invalid-version"));
            return;
        }
        try {
            if (VersionComparator.compare(remoteVersion, currentVersion()) > 0) {
                Result update = new Result(State.UPDATE_AVAILABLE, remoteVersion, Messages.raw("update.available"));
                result.set(update);
                plugin.getLogger().warning("检测到 IdlePool 新版本 " + remoteVersion
                        + "，当前版本 " + currentVersion() + "。下载：" + RELEASES_URL);
            } else {
                result.set(new Result(State.UP_TO_DATE, remoteVersion, Messages.raw("update.up-to-date")));
                plugin.getLogger().info("IdlePool 更新检查完成：当前已是最新版本（" + remoteVersion + "）。");
            }
        } catch (IllegalArgumentException exception) {
            fail(Messages.raw("update.compare-failed", java.util.Map.of("error", String.valueOf(exception.getMessage()))));
        }
    }
